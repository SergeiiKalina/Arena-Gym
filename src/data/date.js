export const itemData = [
    { id: 20, img: "./images/gallery01.jpg", title: "massage office" },
    { id: 21, img: "./images/gallery02.jpg", title: "massage office" },

    { id: 22, img: "./images/gallery03.jpg", title: "massage office" },
    { id: 23, img: "./images/gallery04.jpg", title: "massage office" },
    { id: 0, img: "./images/gallery05.jpg", title: "massage office" },
    { id: 1, img: "./images/gallery06.jpg", title: "massage office" },
    { id: 2, img: "./images/gallery07.jpg", title: "massage office" },
    { id: 5, img: "./images/gallery08.jpg", title: "massage office" },
]
export const itemMenu = [
    {
        id: 101,
        title: "Наші программи",
        src: "#CardServices",
    },
    {
        id: 102,
        title: "Тренери",
        src: "#CardCoach",
    },
    {
        id: 103,
        title: "Абоніменти",
        src: "#Card",
    },

    {
        id: 106,
        title: "Галерея",
        src: "#Gallery",
    },
]

export const slideArr = [
    { id: 0, title: "", img: "./images/slider01.jpg", alt: "photo" },
    { id: 1, title: "", img: "./images/slider02.png", alt: "photo" },
    { id: 2, title: "", img: "./images/slider03.jpg", alt: "photo" },
    { id: 3, title: "", img: "./images/slider04.jpg", alt: "photo" },
]

export const coachArray = [
    {
        id: 0,
        name: "ЯКУЩЕНКО ОЛЕКСІЙ",
        jobTitle:
            "Силовий та функціональний тренінг. Дієтологія. Бодібілдинг. ",

        img: "./images/coach06.jpg",
        linkOnForm: "Запис до Якущенко Олексій",
    },
    {
        id: 1,
        name: "КАЛИНА СЕРГІЙ",
        jobTitle: "Силовий та функціональний тренінг. Crossfit. Реабілітація.",

        img: "./images/coach04.jpg",
        linkOnForm: "Запис до Калина Сергій",
    },
    {
        id: 2,
        name: "ТАРАН АНАСТАСІЯ",
        jobTitle:
            "Силовий та функціональний тренінг. Реабілітація. Дієтологія.",

        img: "./images/coach01.jpg",
        linkOnForm: "Запис до Таран Анастасія",
    },
    {
        id: 3,
        name: "ГУРЗА КАРОЛІНА",
        jobTitle: "Силовий та функціональний тренінг. Дієтологія.",

        img: "./images/coach08.jpg",
        linkOnForm: "Запис до Гурза Кароліна",
    },
    {
        id: 4,
        name: "БАСНЕВ ВІТАЛІЙ",
        jobTitle: "Силовий та функціональний тренінг. Реабілітація. Mind Body.",

        img: "./images/coach05.jpg",
        linkOnForm: "Запис до Баснев Віталій",
    },
    {
        id: 5,
        name: "ШУЛЯК ТИМОФІЙ",
        jobTitle:
            "Силовий тренінг. Бодібілдинг. Практикуючий лікар – анестезист.",

        img: "./images/coach07.jpg",
        linkOnForm: "Запис до Шуляк Тимофій",
    },
    {
        id: 6,
        name: "ОЛЬГА СИМОНОВА",
        jobTitle:
            "Силовий тренінг. Бодібілдинг. Практикуючий лікар – анестезист.",

        img: "./images/coach02.jpg",
        linkOnForm: "Запис до Ольга Симована",
    },
    {
        id: 7,
        name: "ШУЛЯК ТИМОФІЙ",
        jobTitle:
            "Силовий тренінг. Бодібілдинг. Практикуючий лікар – анестезист.",

        img: "./images/coach03.jpg",
        linkOnForm: "Запис до Шуляк Тимофій",
    },
]

export const cardServicesArr = [
    {
        id: 0,
        header1: "ТРЕНАЖЕРНИЙ",
        header2: "ЗАЛ",
        img: "./images/home-11-img-1.jpg",
        paragraph1: "",
        paragraph2: "",
        list: [
            "Простір для ефективних та комфортних тренувань.",
            "Якісне сучасне  обладнання",
            "Сучасний LOFT-дизайн",
            "Надпотужна вентиляція",
        ],
        selector: "half-width two",
        nameButton: "Записатись на заняття",
        linkOnForm: "Запис в трежерний зал",
    },
    {
        id: 1,
        header1: "ГРУПОВІ",
        header2: "ЗАНЯТТЯ",
        img: "./images/home-11-img-2.jpg",
        paragraph1: ``,
        paragraph2: ``,
        selector: "half-width two",
        nameButton: "Дивитись Розклад",
        list: [
            "Груповий фітнес в ArenaGym:",
            "Різноманітні програми",
            "Командний дух",
            "Постійний ритм",
            "Командний дух",
            "Постійний ритм",
            "Контроль інструктора",
            "Оптимальне поєднання для досягнення ваших цілей!",
        ],
        src: "/schedule",
    },
    {
        id: 2,
        header1: "ПЕРСОНАЛЬНІ",
        header2: "ТРЕНУВАННЯ",
        img: "./images/home-11-img-3.jpg",
        paragraph1: `Персональні тренування - гарантовані результати!
        Індивідуальні програми з урахуванням побажань та рівня підготовки.`,
        paragraph2: `Професійні рекомендації по харчуванню.
        Особистий тренер - ключ до ефективних тренувань та здорового способу життя.
        З тренером шлях до мети стає коротшим, ефективнішим та безпечнішим!`,
        selector: "half-width two",
        nameButton: "Записатись на заняття",
        linkOnForm: "Запис на персональні тренування",
    },
    {
        id: 3,
        header1: "",
        header2: "ФІТНЕС БАР",
        img: "./images/cardService05.jpg",
        paragraph1: `ArenaGym запрошуємо  стильного та затишного фітнес бару.
        Збалансоване харчування - ключовий фактор успіху.
        Смачні та корисні страви, протеїнові коктейлі, улюблені напої`,
        paragraph2: `Насолоджуйтеся відпочинком та відновлюйте сили  у фітнес барі ArenaGym.`,
        selector: "half-width two",
        nameButton: "",
    },
    {
        id: 4,
        header1: "СТУДІЯ КОРЕКЦІЇ",
        header2: "ФІГУРИ LADIES CLUB",
        img: "./images/IMG_6714.jpg",
        paragraph1: `LADIES CLUB: Студія корекції фігури та масажу.
        Досвідчені майстри та авторська програма корекції.
        Зменшення обсягів, виточування силуету, антицелюлітний ефект.`,
        paragraph2: `Поліпшення тонусу шкіри та згладжування стрій.
        Стимулювання метаболізму для вражаючих результатів.
        Масаж – ідеальне доповнення до спорту та фізичних навантажень.`,
        list: "",
        selector: "half-width two",
        nameButton: "",
    },
    {
        id: 5,
        header1: "КРОССФІТ",
        header2: "",
        img: "./images/cardservice06.jpg",
        paragraph1: ``,
        paragraph2: ``,
        list: [
            "Будемо розвивати такі якості на заняттях:",
            "Витривалість серцево-судинної та дихальної систем",
            "Загальна Витривалість",
            "Сила",
            "Гнучкість",
            "Потужність",
            "Швидкість",
            "Координація",
            "Спритність",
            "Баланс",
            "Точність",
        ],
        selector: "half-width two",
        nameButton: "Записатись",
        linkOnForm: "Запис на Кроссфіт",
    },
]

export const cardArray = [
    {
        id: 0,
        name: "MORNING",
        coast: "2000uan",
        description: [
            "Пн-Пт 08:00 – 17:00",
            "Сб-Нд 09:00 – 14:00",
            "Тренажерний зал",
            "Кардіотеатр",
            "Зона Crossfit",
            "Призупинка до 30 днів",
            "-10% персональні тренування",
        ],
        linkOnForm: "Замовити Morning карту",
    },
    {
        id: 1,
        name: "FULL DAY",
        coast: "3000uan",
        description: [
            "Пн-Пт 08:00 – 21:00",
            "Сб-Нд 09:00 – 19:00",
            "Тренажерний зал",
            "Кардіотеатр",
            "Зона Crossfit",
            "Призупинка до 30 днів",
            "-10% персональні тренування",
        ],
        linkOnForm: "Замовити Full day карту",
    },
    {
        id: 2,
        name: "ALL INCLUSIVE",
        coast: "1500uan",
        description: [
            "Пн-Пт 08:00 – 17:00",
            "Сб-Нд 09:00 – 14:00",
            "Групові заняття – БЕЗЛІМІТНО!",
            "Тренажерний зал",
            "Кардіотеатр",
            "Зона Crossfit",
            "Призупинка до 30 днів",
            "-10% персональні тренування",
        ],
        linkOnForm: "Замовити All Inclusive карту",
    },
    {
        id: 3,
        name: "GROUP TRAINING",
        coast: "1500uan",
        description: [
            "Пн-Пт 08:00 – 17:00",
            "Сб-Нд 09:00 – 14:00",
            "*за розкладом",
            "Силові тренування",
            "Аєробні тренування",
            "Реабілітаційні тренування",
            "Функціональні тренування",
            "-10% персональні тренування",
        ],
        linkOnForm: "Замовити Group Training карту",
    },
]

export const tableSchedule = [
    {
        day: "Понеділок",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "4165bb55-cf27-4b10-a74a-97c8c531b1b6",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "875caa80-8671-43a2-9e18-b88c845771d4",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "ed026a0b-80ff-4d77-a2e5-0bf030d1faa1",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "ed44eaac-695e-47e2-ab29-dd9a6aeceda7",
            },
            {
                time: "12:00",
                name: "Аренда",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "23c44f82-40f0-471d-8cd9-60f2b3d635ce",
            },
            {
                time: "13:00",
                name: "Аренда",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "b0ec61c9-849f-4e2e-8910-36ccf1e1c4c0",
            },
            {
                time: "14:00",
                name: "Аренда",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "40023893-cf3b-4cef-9d1e-75d0031315de",
            },
            {
                time: "15:00",
                name: "Аренда",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "d06eb0ea-be23-4a7f-96d0-f6b4d65fe321",
            },
            {
                time: "16:00",
                name: "Аренда",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "e5b96f7f-903b-45ba-809b-d850d65d4f60",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "29f16bc9-d824-45ff-aa4c-f370edd081c5",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "f5cd6a64-832d-4cb2-8521-5e8e0133ebef",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "c42bea41-4fcb-4dd6-a1e9-3e446c8a66cb",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "49cbf8cc-a671-4e33-a06a-3786a9456a44",
            },
        ],
        id: "46068e88-3e3d-48b9-8c1b-adfd705c6cec",
    },
    {
        day: "Вівторок",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "1a95c3eb-873b-4bf0-8f41-1114893d421b",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "94ff8b14-815e-4f62-b285-181c463d147b",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "a167d3fd-b626-4539-9ca6-18cec06f857b",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "7763f482-1971-448e-b2a1-b44e91d7fc98",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "b697595b-de39-4d4a-9fce-b1caf6b0f2a0",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "1882b332-27e8-44cb-84f3-76c175b8f1c1",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "53be9e74-a0ff-489b-b446-efd062f4883b",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "de043fdf-9fcc-42e8-aef4-381fa903f5a5",
            },
        ],
        id: "8f194089-8f3f-4dd5-9ddb-5999dc17db54",
    },
    {
        day: "Середа",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "4a3644ee-5e6c-4061-813a-aba95a80d776",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "2878db6c-2870-4ae2-b006-b18f74dc7ba6",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "663e5129-04b5-4cb2-9313-52a31e1df60b",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "708fe0da-76d4-4560-825a-3fae9001b50e",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "5ef497cc-bc6e-49c8-a42d-2bcb56f1f0ee",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "14c97b44-f7b4-4859-82e3-a17b6c512ff1",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "2c7343c7-225c-46a1-b1aa-d3668775eb22",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "5bc6c871-5f33-4a5f-a620-52f3a8cff9f1",
            },
        ],
        id: "1847a555-8dd5-43ad-ab24-7dfa75bd12f7",
    },
    {
        day: "Четверг",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "683bbaeb-39fa-41ed-b5e4-f3eddabc0fcb",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "8a418a1a-c6dd-4022-b038-3d007abb7c5c",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "a3cd40af-0637-451a-a249-996c5e81d814",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "e72da0ca-bb22-4743-bd0f-8090388d547e",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "053b4cf7-d1d7-4122-bbbd-9fc9ed608f16",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "1d08903c-0be6-4727-82ea-20e73722838c",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "30b6e129-83f4-4f1d-88b6-4a66fa35a211",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "c1f4ab6c-e5ae-4e7c-9c4b-e391148550ef",
            },
        ],
        id: "f715b9c6-8a44-4975-972c-87edf1bc37b9",
    },
    {
        day: "П'ятниця",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "466b2269-96cd-4b2f-b5d7-82e16808e7e2",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "a39ca95d-333f-4352-a8ea-c89427f24d62",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "51ade206-23d8-4b4e-86d4-8e6ee35d395d",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "c6b2e84f-c602-4264-b707-604acd3fcbeb",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "791039f9-fc38-465f-a3a1-775e768cdc0b",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "70772e62-1f09-4d14-8b7c-7521929d126f",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "10f1b405-1df0-4f89-a585-3ae2694e5468",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "54e49f32-8fbd-4c10-aa6b-073c5a34eb30",
            },
        ],
        id: "c6bcd60a-1eb2-4b87-8dd0-689a652b115d",
    },
    {
        day: "Субота",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "6ce15db8-eb9e-4228-b575-a830f7f442f0",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "86e53222-c202-4286-a2c7-dfb3b754fa32",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "ed8e9a26-10ed-41a1-a716-e5f0ceb8eedc",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "09b58fee-72c6-4b15-afa1-9ae21e97461f",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "7a96c575-9cd0-4ba1-bf1d-06ea75922156",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "5dd66f79-73ef-498e-9fb5-6c88ae4b90c2",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "e3d7b813-c7f5-4a5d-a53d-8364e6191299",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "38134c7f-b232-4efc-95cc-0e9c7e434d13",
            },
        ],
        id: "1fdde945-5f8e-490c-a902-bf8c147bfcc0",
    },
    {
        day: "Неділя",
        training: [
            {
                time: "8:00",
                name: "ABS",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "d5c92959-c0a8-4755-8f41-0f2c98289b8f",
            },
            {
                time: "9:00",
                name: "Crossfit",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "8fdc54f7-da29-4226-bc2c-82614013941c",
            },
            {
                time: "10:00",
                name: "Fitness",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "448e35ce-2cf5-41cf-9278-20d6d810bc3c",
            },
            {
                time: "11:00",
                name: "huitnes",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "e4aa022a-65c4-4049-be2c-691edb1587cd",
            },
            {
                time: "17:00",
                name: "Pilates",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "9b862dea-0e8a-4530-813b-7e35758ebb43",
            },
            {
                time: "18:00",
                name: "stretching",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "a4c2f263-0b7b-4444-8b99-8ea85db1967e",
            },
            {
                time: "19:00",
                name: "physical education",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "265320ef-9c19-4ecf-ac7f-84061b103e1c",
            },
            {
                time: "20:00",
                name: "functional training",
                description:
                    "Lorem ipsum dolor sit, amet consectetur\nadipisicing elit. Voluptas at distinctio nobis\nmagni ea facilis earum qui.",
                id: "04a39c3e-3c01-4423-a221-e7a6c940282a",
            },
        ],
        id: "8bac639a-4bdd-4e7b-80ea-1dc4a39e0c2e",
    },
]
