          ...
          <div className="schedule_table">
            {tableSchedule.map((el, index) => {
              if (el.training && el.training.length !== 0) {
                ...
                for (let i = 0; i < 10; i++) {
                  const hour = firstTimeOfDay;
                  const currentItem = el.training.find((item) => {
                    return parseInt(item.time.split(':')[0]) === hour;
                  });

                  if (currentItem) {
                    rows.push(
                      <div className="schedule_row_item" key={currentItem.id}>
